...
class User(BaseModel, AbstractBaseUser, PermissionsMixin):
    ...
    last_name = models.CharField(_('Last Name'), max_length=128, null=True, blank=True,
                                 default=_('No Name'))
    role = models.CharField(_('Role'), max_length=20, choices=Role.choices, default=Role.DOCTOR)
    expertise = models.CharField(_('Expertise'), max_length=50, null=True, blank=True)
    country = models.CharField(_('Country'), max_length=20, null=True, blank=True)
    is_active = models.BooleanField(_('Active'), default=True)
    is_admin = models.BooleanField(_('Admin'), default=False)
    is_verified = models.BooleanField(_('Verify'), default=False)
    token = models.CharField(_("Secret Token"), max_length=64, null=True, blank=True, unique=True, editable=False)
    objects = UserManager()
...
def generate_patient_code():
    ...
    return f"PAT-{today}-{uuid.uuid4().hex[:6].upper()}"


class PatientModel(BaseModel):
    Gender = UserGenderEnum

    patient_code = models.CharField(_('Patient Code'), max_length=30, unique=True, blank=True)
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True,
                                   related_name='patients_created', verbose_name=_('Created By'))
    full_name = models.CharField(_('Full name'), max_length=150)
    age = models.CharField(_('Age'), null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.patient_code:
            self.patient_code = generate_patient_code()
